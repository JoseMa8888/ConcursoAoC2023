...
def calibrationsum(l : list[int]) -> int:
    sum = 0
    for i in l:
        x = calibration(i)
        sum += x 
...
def findfirstnumber(s : str) -> int:
    esta = False
    i = 0
    while i < len(s) and not esta:
        if s[i].isdigit():
            esta = True
        else:
            i = i + 1
...
def test():
    cadena_vertical = """dssmtmrkonedbbhdhjbf9hq
...
449three45three"""
    ...
    lista_resultado = cadena_vertical.split('\n')
    print(str(calibrationsum(lista_resultado)))
